...
function updateNotifBadge() {
    ...
    if (currentUser && currentUser.role === 'owner') {
        ...
        count += getDailyMeds().filter(d => d.userId === currentUser.id && !d.takenToday).length;
    }
...
async function renderNotifModal() {
    ...
    } else { contentHTML = '<p>No notifications.</p>'; }
    tabsDiv.innerHTML = tabsHTML;
    contentDiv.innerHTML = contentHTML;
    window._currentNotifTab = currentUser?.role === 'owner' ? 'stock' : 'medications';
}
...
async function registerUser(role = null, isOwnerAction = false) {
    ...
    const selectedRole = isOwnerAction ? (document.getElementById('regRole')?.value || 'user') : role;
...
function forgotPassword() { showNotification('Reset link sent (demo).', 'info', false); }
...
function renderSellerTab(tab) {
    const content = document.getElementById('dashboardContent');
    if (tab === 'orders') { const orders = getOrders(); content.innerHTML = `<h2>📦 Orders</h2>${orders.length ? `<div class="table-wrapper"><table>...</table></div>` : '<p>No orders.</p>'}`; }
...
async function approveVerification(userId) {
    const db = window.db;
    if (db && userId) { const userRef = firebaseDoc(db, "users", userId); await firebaseUpdateDoc(userRef, { verified: true }); if (currentUser && currentUser.id === userId) { currentUser.verified = true; localStorage.setItem('currentUser', JSON.stringify(currentUser)); } }
    let reqs = getVerificationRequests(); const req = reqs.find(r => r.userId === userId && r.status === 'pending'); if (req) req.status = 'approved'; saveVerificationRequests(reqs);
    renderAdminTab('verifications'); showNotification('User verified!', 'success', false); updateNotifBadge();
}
async function rejectVerification(userId) {
    let reqs = getVerificationRequests(); const req = reqs.find(r => r.userId === userId && r.status === 'pending'); if (req) req.status = 'rejected'; saveVerificationRequests(reqs);
    renderAdminTab('verifications'); showNotification('Rejected.', 'info', false); updateNotifBadge();
}
...
document.addEventListener('keydown', e => { if (e.key === 'Escape') { closeLoginModal(); closeCartModal(); closeMedicineDetail(); closeAccountDropdown(); closeCheckoutModal(); closeNotifModal(); } });
init();