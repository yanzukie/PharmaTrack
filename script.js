/* ==============================
   ...
   Mobile Dropdown + Admin Overview
   ============================== */
...
// ---------- DASHBOARD (Role-based sidebar with mobile dropdown) ----------
function renderAdminOverview() {
    const content = document.getElementById('dashboardContent');
    if (!currentUser) return;
    const totalMeds = getMedicinesLocal().length;
    const pendingVerif = getVerificationRequests().filter(r => r.status === 'pending').length;
    ...
    content.innerHTML = `
        ...
        <div class="stats-row">
            <div class="stat-card">
                ...
                <div class="stat-label">Medicines in Inventory</div>
            </div>
            <div class="stat-card">
                <div class="stat-number">${pendingVerif}</div>
        ...
        <p>Use the sidebar (or dropdown on mobile) to manage inventory, users, orders, and more.</p>
    `;
}

function renderDashboard() {
    const sidebar = document.getElementById('dashboardSidebar');
    const mobileNav = document.getElementById('dashboardMobileNav');
    ...
    if (role === 'owner') {
        tabs = [
            ...
            { id: 'medicines', label: '💊 Inventory Management' },
            { id: 'users', label: '👥 Manage Users' },
            { id: 'orders', label: '📦 View Orders' },
            { id: 'verifications', label: '🔔 Verify Users' },
            { id: 'doctors', label: '👨‍⚕️ Manage Doctors' },
            { id: 'adminSettings', label: '⚙️ Settings' },
            { id: 'logout', label: '🚪 Logout' }
        ];
        extraButton = `<button onclick="openAddUserModal()" class="btn btn-sm btn-outline" style="margin:8px 20px;">➕ Add Staff/User</button>`;
    } else if (role === 'staff') {
        tabs = [
            { id: 'overview', label: '📊 Overview' },
            { id: 'medicines', label: '💊 Manage Inventory' },
            { id: 'orders', label: '📦 View Orders' },
            { id: 'doctors', label: '👨‍⚕️ View Doctors' },
            { id: 'staffSettings', label: '⚙️ Settings' },
            { id: 'logout', label: '🚪 Logout' }
        ];
    } else if (role === 'seller') {
        tabs = [
            { id: 'orders', label: '📦 Manage Orders' },
            { id: 'sellerSettings', label: '⚙️ Settings' },
            { id: 'logout', label: '🚪 Logout' }
        ];
    } else if (role === 'doctor') {
        tabs = [
            { id: 'appointments', label: '📅 Appointments' },
            { id: 'patientMeds', label: '💊 Patient Medications' },
            { id: 'doctorSettings', label: '⚙️ Settings' },
            { id: 'logout', label: '🚪 Logout' }
        ];
    } else {
        tabs = [
            { id: 'overview', label: '📊 Overview' },
            { id: 'dailyMeds', label: '💊 Daily Medications' },
            { id: 'appointments', label: '📅 Appointments' },
    ...
    const defaultTab = role === 'owner' ? 'overview'
                     : role === 'staff' ? 'overview'
                     : role === 'seller' ? 'orders'
    ...
    tabs.forEach(tab => {
        if (tab.id === 'logout') {
            sidebarHTML += `<button onclick="confirmLogout()" style="color:var(--danger);">${tab.label}</button>`;
        } else {
            sidebarHTML += `<button class="${tab.id === defaultTab ? 'active-tab' : ''}" onclick="switchDashboardTab('${tab.id}')">${tab.label}</button>`;
        }
    });
    if (extraButton) sidebarHTML += extraButton;
    sidebar.innerHTML = sidebarHTML;

    let mobileHTML = '';
    if (extraButton) {
        mobileHTML += `<div style="margin-bottom:8px;">${extraButton}</div>`;
    }
    mobileHTML += `<select id="dashboardMobileSelect" onchange="switchDashboardTab(this.value)">`;
    tabs.forEach(tab => {
        if (tab.id !== 'logout') {
            mobileHTML += `<option value="${tab.id}" ${tab.id === defaultTab ? 'selected' : ''}>${tab.label}</option>`;
        }
    });
    mobileHTML += `</select>`;
...
function switchDashboardTab(tab) {
    ...
    const mobileSelect = document.getElementById('dashboardMobileSelect');
    if (mobileSelect) mobileSelect.value = tab;

    if (tab === 'logout') { confirmLogout(); return; }

    // Admin overview
    if (tab === 'overview' && (currentUser.role === 'owner' || currentUser.role === 'staff')) {
        renderAdminOverview();
        return;
    }
...
function renderSellerTab(tab) {
    const content = document.getElementById('dashboardContent');
    if (tab === 'orders') { const orders = getOrders(); content.innerHTML = `<h2>📦 Orders</h2>${orders.length ? `<div class="table-wrapper"><table>...</table></div>` : '<p>No orders.</p>'}`; newOrderCount = 0; updateNotifBadge(); }
